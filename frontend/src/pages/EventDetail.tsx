...
export default function EventDetail() {
  ...
  const isRegistered = user && event?.attendees?.some(
    (a: any) => a._id === user._id || a === user._id
  );
  ...
  const handleRegister = async () => {
    ...
    try {
      if (isRegistered) {
        ...
        setEvent((prev) =>
          prev
            ? { ...prev, attendees: res.data.attendees || [...prev.attendees, user] }
            : prev
        );
      }
    } catch (err: any) {
      setRegistrationError(err.response?.data?.message || "Action failed");
    } finally {
  ...
  if (!event) {
    return (
      <div className="max-w-4xl mx-auto flex flex-col items-center justify-center py-20 text-gray-500">
        ...
        <Link to="/events" className="mt-2 text-blue-400 hover:text-blue-300 transition-colors">
          &larr; Back to Events
        </Link>
      </div>
    );
  ...
  const catColor = categoryColors[event.category?.toLowerCase()] || "bg-gray-500/20 text-gray-400 border-gray-500/30";
  const statColor = statusColors[event.status] || "bg-gray-500/20 text-gray-400 border-gray-500/30";
  const attendeePercent = event.maxAttendees ? Math.min((event.attendees.length / event.maxAttendees) * 100, 100) : 0;
      ...
      <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 overflow-hidden">
        <div className="p-6 sm:p-8 space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-3 py-0.5 rounded-full text-xs font-medium border ${statColor}`}>
                  {event.status}
                </span>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-5 bg-gray-900/50 rounded-xl border border-gray-700/30">
            ...
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Organizer</p>
              <p className="text-white font-medium">{event.organizer || "GBAABW"}</p>
            </div>
          </div>
          ...
          <div className="p-6 bg-gray-900/50 rounded-xl border border-gray-700/30">
            ...
            {!token && (
              <p className="text-gray-500 text-sm mt-3">
                <Link to="/login" className="text-blue-400 hover:text-blue-300 transition-colors">Log in</Link> to register for this event.
              </p>
            )}
          </div>
          ...
          {event.attendees.length > 0 && (
            <div>
              <h3 className="text-lg font-bold text-white mb-4">Attendees ({event.attendees.length})</h3>
