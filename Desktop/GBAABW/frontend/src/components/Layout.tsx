        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-2">
              ...
              >
                <img src="/asset/Picture1.png" alt="GBAABW logo" className="h-10 w-auto rounded-full" />
              </Link>
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute top-full right-0 mt-1 w-52 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50">
          <Link to={user?.role === "superadmin" ? "/superadmin/dashboard" : user?.role === "admin" ? "/admin/dashboard" : "/dashboard"} onClick={() => setOpen(false)} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            Dashboard
          </Link>
          <Link to="/profile" onClick={() => setOpen(false)} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            Profile
          </Link>
          <Link to="/my-events" onClick={() => setOpen(false)} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            My Events
          </Link>
          <Link to="/notifications" onClick={() => setOpen(false)} className="flex items-center justify-between px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            Notifications
            {unreadCount > 0 && (
...
function MobileMenu({
  ...
  const dashboardPath = user?.role === "superadmin" ? "/superadmin/dashboard" : user?.role === "admin" ? "/admin/dashboard" : "/dashboard";
  ...
  return (
    <div className="fixed inset-y-0 left-0 w-64 max-w-[75vw] z-50 lg:hidden bg-white dark:bg-gray-900 shadow-2xl flex flex-col animate-slide-in-left">
      <div className="flex items-center justify-between px-3 h-12 border-b border-gray-200 dark:border-gray-700 shrink-0">
        <img src="/asset/Picture1.png" alt="GBAABW" className="h-7 w-7 rounded-full" />
        <button onClick={closeMenu} className="p-1.5 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto py-1.5 px-2 space-y-0.5">
        {user && (
          <>
            ...
            <Link to={dashboardPath} onClick={closeMenu} className={linkClass(dashboardPath)}>
            ...
            {user?.role === "superadmin" && (
              <Link to="/superadmin/users" onClick={closeMenu} className={linkClass("/superadmin")}>
                <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 p-3 shrink-0">
        {user ? (
          ...
          <div className="flex flex-col gap-1.5">
            <Link to="/login" onClick={closeMenu} className="w-full text-center px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">
              Login
            </Link>
            <Link to="/register" onClick={closeMenu} className="w-full text-center px-3 py-2 text-xs font-semibold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 rounded-lg transition-colors">
              Register
            </Link>
          </div>
        )}
