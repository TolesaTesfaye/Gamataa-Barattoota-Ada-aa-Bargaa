...
export const HighSchoolOverview: React.FC<HighSchoolOverviewProps> = ({
  ...
  const tracks = [
    {
      title: "Start Lesson",
      subtitle: "Jump into Learning Center",
      icon: Brain,
    ...
    {
      title: "Practice Hub",
      subtitle: "Explore all materials",
      icon: BookOpen,
    ...
    {
      title: "Exam Mode",
      subtitle: "Focus on top resources",
      icon: Target,
      action: () => setActiveTab("hub"),
  ...
  const highlights = [
    {
      label: "Grade",
      value: activeGrade.replace("_", " ").toUpperCase(),
      icon: GraduationCap,
    },
    ...
    <div className="space-y-3 md:space-y-6 pb-6 md:pb-8 animate-in fade-in duration-700">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 items-center">
          <div>
            <p className="inline-flex items-center gap-1.5 px-2 py-0.5 md:px-3 md:py-1 rounded-full bg-white/15 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2 md:mb-4">
              <Sparkles className="w-3 h-3 md:w-3.5 md:h-3.5" /> Personalized
              High School Space
            </p>
            ...
            <p className="mt-2 md:mt-4 text-xs md:text-sm lg:text-base text-blue-100/90">
              Your {activeGrade.replace("_", " ").toUpperCase()} dashboard is
              ready with smart recommendations, trending materials, and focused
              learning tracks.
            </p>
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-3">
            {highlights.map((item) => (
              <div
                key={item.label}
                className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-lg md:rounded-xl p-3 md:p-4"
              >
                ...
                <p className="text-[10px] md:text-xs text-blue-100/80 uppercase tracking-widest">
                  {item.label}
                </p>
              </div>
            ))}
      ...
      <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-4 px-2 md:px-4">
        {tracks.map((track) => (
          ...
          >
            ...
            <h3 className="font-bold text-xs md:text-lg mb-1 md:mb-1 leading-tight">
              {track.title}
            </h3>
      ...
      <div className="bg-white dark:bg-slate-900/60 border-t border-b border-slate-200 dark:border-slate-800 p-3 md:p-6 shadow-sm">
        <div className="flex items-center justify-between mb-3 md:mb-5 px-1">
          <h2 className="text-sm md:text-xl font-bold text-slate-900 dark:text-white">
            Trending Materials
          </h2>
          <button
            onClick={() => setActiveTab("hub")}
        ...
        {homeLoading ? (
          <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-[200px] md:h-[350px] rounded-xl md:rounded-xl bg-slate-100 dark:bg-slate-800 animate-pulse"
              />
            ))}
          </div>
          ...
          <div className="text-center py-8 md:py-16 bg-slate-50 dark:bg-slate-800/30 rounded-lg md:rounded-xl border border-dashed border-slate-200 dark:border-slate-700 mx-1">
            ...
            <p className="text-slate-500 text-xs md:text-base">
              No resources yet. New content will appear here.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4">
      ...
      <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-4 px-2 md:px-4">
        {achievements.map((achievement, index) => (
          <div
            key={index}
            className="border-t border-b border-slate-200 dark:border-slate-800 p-3 md:p-5 bg-white dark:bg-slate-900/60 rounded-lg hover:shadow-lg transition-all"
          >
            <div className="flex flex-col md:flex-row items-start md:items-center gap-1 md:gap-3 mb-2">
              <achievement.icon
                className={`w-5 h-5 md:w-6 md:h-6 ${achievement.color}`}
              />
              <h4 className="text-[9px] md:text-xs font-semibold uppercase tracking-wide text-slate-500 leading-tight">
                {achievement.label}
              </h4>
            </div>
            <p className="text-base md:text-3xl font-black text-slate-900 dark:text-white">
              {achievement.value}
            </p>
          </div>
        ))}
